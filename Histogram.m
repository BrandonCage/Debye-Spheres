...
Con=me/k/Temp;
xtemp=[0:300:15000];
ytemp=N*300*Con*xtemp.*exp(-Con*xtemp.^2/2);
vtot=[(vxp(1:N).^2+vyp(1:N).^2).^.5;30000];
...
dE=[dE;.5*sum(massmat*(vxp.^2+vyp.^2),'all')+.5*e0*sum(Ex1.^2+Ey1.^2,'all')-totEn];
totEn=.5*sum(massmat*(vxp.^2+vyp.^2),'all')+.5*e0*sum(Ex1.^2+Ey1.^2,'all');
Temp=(sum((xp.^2+vyp.^2),'all')*me/N)/k;

...
rho=sum(emat.*weight(:,1:N),2)/dx^2+rho_back;%rho of each grid point
%test=sum(rho1,'all')
...
j=find(out);
if isempty(j)==0
    rho_back=rho_back+sum(emat(j).*weight(:,j),2)/dx^2;
    weight(:,j)=[];
end
...
%plot(UdUE)
%plot(LdUE)
legend("Energy","Potential","Kinetic","Energy with Lost Energy")
hold off

...
bins=10;
s1cdx=linspace(2*pi/bins,2*pi,bins);
s1cdy=zeros(bins,1);
s2cdx=linspace(2*pi/bins,2*pi,bins);
s2cdy=zeros(bins,1);
...
g1=cat(2,ceil(xpc/dx-.5),ceil(ypc/dx-.5));%put on grid points
g=[g1;g1+1];%put on grid points
fraz1=((1-abs(xpc/dx-g1(1:NG^2,1)+.5))'*diag((1-abs(ypc/dx-g1(1:NG^2,2)+.5))'))';
fraz2=((abs(xpc/dx-g1(1:NG^2,1)+.5))'*diag((1-abs(ypc/dx-g1(1:NG^2,2)+.5))'))';
fraz3=((1-abs(xpc/dx-g1(1:NG^2,1)+.5))'*diag((abs(ypc/dx-g1(1:NG^2,2)+.5))'))';
fraz4=((abs(xpc/dx-g1(1:NG^2,1)+.5))'*diag((abs(ypc/dx-g1(1:NG^2,2)+.5))'))';
...
%Capacity Matrix
Inc1=(((xpc-sxc1).^2+(ypc-syc1).^2)<sr1^2);
Inc2=(((xpc-sxc2).^2+(ypc-syc2).^2)<sr2^2);
ematc1temp=zeros(NG^2,1);
ematc2temp=zeros(NG^2,1);
ematc1temp(Inc1)=1;
ematc2temp(Inc2)=1;
ematc1=ematc1temp;
ematc2=ematc2temp;
...
indices=find(ematc1);
Bprime=zeros(length(indices));
parfor n=1:length(indices)
    rhotemp=zeros(NG^2);
    rhotemp(indices(n))=1/dx^2;
...
%PUT-PARTICLES-ON-A-RING_BRANDON
